...
def prepare_data_vector(data, N, n_onehot):
    ...
    for n,(i,j) in enumerate(data.edge_index.T):
        ...
        adj[nn,ii,jj] = data.edge_attr[n,:].matmul(torch.tensor([1,2,3,1.5], device=atom_fea.device))
...
def train(qm9, config, output):
    ...
    if config.use_pretrained:
        ...
        data_train = next(iter(DataLoader(qm9[:config.n_data], batch_size = config.n_data, shuffle = False))).to(device=device)
        plt.plot(data_train.y[:,4].cpu(), model(*prepare_data_vector(data_train, config.max_size, config.n_onehot)).cpu().detach().numpy(), ".")
        ...
        data_valid = next(iter(DataLoader(qm9[config.n_data:config.n_data + config.n_data//10], batch_size = config.n_data//10, shuffle = False))).to(device=device)
        plt.plot(data_valid.y[:,4].cpu(), model(*prepare_data_vector(data_valid, config.max_size, config.n_onehot)).cpu().detach().numpy(),".")
