...
def weights_to_model_inputs_AO(fea_h, adj_vec, config):
    ...
    atom_fea = sig(config.sig_strength*(2*fea_h - 1.0)).T

    atom_fea = torch.cat([gauss(adj_sum, 1.0)*atom_fea, gauss(adj_sum, 4.0), gauss(adj_sum, 3.0), gauss(adj_sum, 3.0), gauss(adj_sum, 1.0)*(1-atom_fea), gauss(adj_sum, 0.0)], dim=1).unsqueeze(0)
...
def initialize_AO(qm9, config, output, i, device="cpu"):
    ...
    if config.start_from == "random":
        ...
        fea_h = torch.rand((1, N), device=device)
...
def invert(target_value, model, fea_h, adj_vec, config, output):
    ...
    for e in loop:
        ...
        integer_fea_losses.append(float(score))
...
def invert_AO(target_value, model, fea_h, adj_vec, config, output):
    """ Backpropagates all the way to the input to produce a molecule with a specific property """
    ...
    for e in loop:
        ...
        atom_fea_ext, adj, constraints, integer_fea, integer_adj = weights_to_model_inputs_AO(fea_h, adj_vec, config)
